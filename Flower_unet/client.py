...
class UnetClient(fl.client.NumPyClient):
    ...
    def fit(self, weights, config):
        ...
        return self.model.get_weights(), len(self.train_loader), len(self.train_loader)
    
    def evaluate(self, weights, config):
        self.model.set_weights(weights)
        loss, iou_score, f1_score = self.model.evaluate(self.val_loader) #need to change metrics
        return len(self.val_loader), loss, iou_score, f1_score
...
def client_fn(cid: str) -> fl.client.NumPyClient:
    ...
    return UnetClient(model, train_loader, val_loader)
