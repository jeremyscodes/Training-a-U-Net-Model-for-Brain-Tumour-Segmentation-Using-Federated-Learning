...
def overlay_images(mri_img, mask_img):
    ...
    vmax_percentile=95
    vmax_val = np.percentile(mri_img, vmax_percentile)
    plt.imshow(mri_img, cmap="bone", origin="lower", alpha=1.0, vmin=0, vmax=vmax_val)  # Display the MRI
...
def plot_results(ds, central_model_dir, fl_model_dir, filename='results_plot.png'):
    ...
    for i in range(5):  # for 5 MRIs
        ...
        # For the next 2 MRIs, select the smallest tumors
        else:
            tumor_sizes = np.sum(np.sum(msk[:,:,:,0], axis=1), axis=1)
            non_zero_tumors = tumor_sizes[(tumor_sizes > 0) & (tumor_sizes > MIN_TUMOR_SIZE)]
            if len(non_zero_tumors) > 0:
                idx = np.where(tumor_sizes == np.min(non_zero_tumors))[0][0]
                print("Tumour size: ", tumor_sizes[idx])
            else:
                # Handle the case where no tumors are above the threshold. 
                # This can either default to the largest tumor or skip the slice, etc.
                idx = np.argmax(np.sum(np.sum(msk[:,:,:,0], axis=1), axis=1))
                MIN_TUMOR_SIZE-=20
        ...
        for j, model in enumerate(all_models):  # for each model
            ...
            plt.xlabel("Dice = {:.4f}".format(dice_score), fontsize=16)
